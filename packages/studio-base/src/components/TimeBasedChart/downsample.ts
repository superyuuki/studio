...
type StateIndex = [index: number, numStates: number];
...
export function downsampleStates(
  ...
  const indices: StateIndex[] = [];

  type Interval = { xPixel: number; labels: Set<string>; index: number };
  ...
  for (const datum of points) {
    ...
    if (interval == undefined || isNew) {
      interval = { xPixel: x, index, labels: new Set([label]) };
      continue;
    }

    interval.labels.add(label);
  }
